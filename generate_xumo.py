# -*- coding: utf-8 -*-
# Xumo playlist + EPG generator
# - Pulls the live channel catalog from Xumo's "Valencia" (web) API list 10006
# - Uses the Android TV endpoints only for "what's on now" (broadcast) and asset stream lookups,
#   because those endpoints tend to expose provider sources (m3u8) in a stable form.
# - Builds an M3U (channels + stream URLs) and XMLTV EPG (program listings) covering EPG_FETCH_DAYS.
# - Writes output into ./playlists as xumo_playlist.m3u and xumo_epg.xml
# - Logging is verbose (DEBUG) for easier troubleshooting.
...
import time
import re
import uuid  # Needed for IFA placeholder and unique IDs in URL templates
import sys
import math  # For calculating total pages (not currently used directly)

# --- Configuration ---
# Two families of endpoints:
# - "Valencia" == web site/app services (used for lists and the EPG aggregator by hour).
# - "Android TV" == mobile/OTT endpoints (often surface playable sources for assets).
ANDROID_TV_ENDPOINT = "https://android-tv-mds.xumo.com/v2"  # Keep for asset/broadcast lookups
VALENCIA_API_ENDPOINT = "https://valencia-app-mds.xumo.com/v2"  # Primary endpoint for lists + EPG
GEO_ID = "us"  # Region gate for the list endpoint (affects catalog)

# --- List IDs ---
# Xumo list "10006" corresponds to the public/live web channel list. We reuse it for the hourly EPG, too.
...
# --- Endpoint URLs ---
# Pull the live channel catalog (metadata, numbers, branding) from Valencia
PRIMARY_CHANNEL_LIST_URL = f"{VALENCIA_API_ENDPOINT}/proxy/channels/list/{PRIMARY_LIST_ID}.json?geoId={GEO_ID}"

# Use Android TV for "what's on now" and asset->provider (stream) resolution
...
# EPG comes from Valencia by day and hour, with chunked offsets. We request asset.title/descriptions
# so we can fully populate XMLTV without re-querying assets individually.
EPG_FETCH_URL_TEMPLATE = (
    f"{VALENCIA_API_ENDPOINT}/epg/{PRIMARY_LIST_ID}/{{date_str}}/{{hour}}.json"
    f"?limit=50&offset={{offset}}&f=asset.title&f=asset.descriptions"
)

# Fallback logo pattern if the channel item doesn’t include one
XUMO_LOGO_URL_TEMPLATE = "https://image.xumo.com/v1/channels/channel/{channel_id}/168x168.png?type=color_onBlack"

# --- Script Settings ---
EPG_FETCH_DAYS = int(os.getenv("EPG_FETCH_DAYS", "2")) # Was -> EPG_FETCH_DAYS = 2          # Number of days of EPG (today + n-1)
MAX_EPG_OFFSET = 350        # Valencia EPG is paginated by offset (step of 50 per inner loop)
HOURS_TO_FETCH = 24         # We iterate through hours 0..23 for each date
API_DELAY_SECONDS = float(os.getenv("API_DELAY_SECONDS", "0.10"))  # was -> API_DELAY_SECONDS = 0.10    # Small delay between calls to be polite and reduce rate-limit risk
OUTPUT_DIR = os.getenv("OUTPUT_DIR", "/data/playlists")  # default to an in-container path  # <— was "playlists"
PLAYLIST_FILENAME = "xumo_playlist.m3u"
EPG_FILENAME = "xumo_epg.xml"
REQUEST_TIMEOUT = int(os.getenv("REQUEST_TIMEOUT", "45"))# was ->REQUEST_TIMEOUT = 45        # Seconds; protects against hung requests

# !!! GitHub Repo Info !!!
# These are only used to populate metadata / relative URL in M3U header (url-tvg)
GITHUB_USER = "BuddyChewChew"
GITHUB_REPO = "xumo-playlist-generator"
GITHUB_BRANCH = "main"
EPG_RAW_URL = f"/{EPG_FILENAME}"  # Kept relative; adjust if serving from raw GitHub or web server

# --- Headers ---
# Simulate a browser for Valencia endpoints (some endpoints are picky about UA/origin/referrer).
WEB_HEADERS = {
...
# Android TV endpoints typically expect a lightweight client UA (okhttp seen in the wild).
ANDROID_TV_HEADERS = {
    'User-Agent': 'okhttp/4.9.3',
}

# --- Logging Setup ---
# DEBUG-level logs for development visibility; switch to INFO in production if desired.
logging.basicConfig(
    level=logging.DEBUG,
...
def fetch_data(url, params=None, is_json=True, retries=2, delay=2, headers=WEB_HEADERS):
    """Fetch a URL with optional params and headers.
    - If is_json=True, returns parsed JSON or None on failure (with decode logging).
    - Retries network errors and most HTTP errors except 401/403/404/429 (non-retriable by default).
    - Logs the built URL and status code for each attempt.
    """
    logging.debug(f"URL: {url}, Params: {params}")
    # logging.debug(f"Headers: {json.dumps(headers)}")  # Enable to inspect headers while debugging
        ...
        try:
            ...
            if is_json:
                ...
                try:
                    ...
                    # Log first 500 chars of body to avoid dumping very large responses
                    logging.error(f"Error decoding JSON. Content: {response.text[:500]}... - {e_final}")
                    if logging.getLogger().level == logging.DEBUG:
                        logging.debug(f"Full Text:\n{response.text}")
                    return None
            else:
                # Return text decoded as UTF-8 (ignoring errors)
                try:
            ...
            if response is not None:
                logging.warning(f"Error Response Content: {response.text[:500]}...")
            # Retry on most 5xx/other codes; do not retry on typical auth/not-found/rate-limited signals
            if attempt < retries and response is not None and response.status_code not in [401, 403, 404, 429]:
                ...
                break  # Non-retriable HTTP error or already handled

        except requests.exceptions.RequestException as e:
            # Network-level issues (DNS, connect, read timeouts, etc.)
...
def parse_iso_datetime(iso_time_str):
    """Parse Xumo ISO-8601 timestamps to timezone-aware UTC datetimes.
    Handles:
      - trailing 'Z'
      - millisecond fragments
      - offsets formatted as +HHMM (no colon)
      - missing offset (assumes +00:00)
    Returns a datetime in UTC or None on failure.
    """
    if not iso_time_str:
    ...
    try:
        ...
        # If there’s a millisecond portion, strip it but preserve the offset at the end
        if '.' in iso_time_str:
            offset_str = ""
            ...
            offset_index = -1
            if plus_index > t_index:
                offset_index = plus_index
            if minus_index > t_index:
                offset_index = max(offset_index, minus_index)
            if offset_index != -1:
        ...
        # Insert colon in +HHMM if needed (Python fromisoformat requires +HH:MM)
        ...
        # If no offset present, assume UTC (+00:00)
...
def format_xmltv_time(dt_obj):
    """Convert a timezone-aware datetime to XMLTV format: YYYYMMDDHHMMSS +HHMM.
    We output in UTC with +0000 offset to keep things consistent across tools.
    """
    ...
    offset_part = dt_obj_utc.strftime('%z')
    offset_part_clean = offset_part.replace(':', '')  # Ensure +HHMM form
...
def ensure_output_dir():
    """Create OUTPUT_DIR if it doesn't exist; propagate exceptions so the caller can abort."""
...
ADD_XMLTV_DOCTYPE = True  # If True, prepend <!DOCTYPE tv SYSTEM "xmltv.dtd"> for XMLTV validators

def save_xml(tree, filepath):
    """Serialize an ElementTree to an uncompressed UTF-8 XML file.
    Adds XMLTV DOCTYPE when ADD_XMLTV_DOCTYPE is True.
    """
    try:
...
{xml_partial_string}'''
            xml_string_to_save = xml_full_string
            logging.debug("Adding DOCTYPE to XML output.")
        else:
            xml_string_to_save = ET.tostring(
                tree.getroot(),
                encoding='unicode',
                method='xml',
        ...
        # NOTE: previously this wrote gzip; now intentionally plain .xml for simplicity/compatibility.
        with open(filepath, 'w', encoding='utf-8') as f:
            f.write(xml_string_to_save)

        logging.info(f"Uncompressed EPG XML file saved: {filepath}")
    except Exception as e:
        logging.error(f"Error writing EPG file {filepath}: {e}")


def save_m3u(content, filepath):
    """Write M3U text to disk in UTF-8."""
    try:
        with open(filepath, 'w', encoding='utf-8') as f:
            f.write(content)
...
def process_stream_uri(uri):
    """Expand placeholder tokens that Xumo returns in provider URIs, then strip unknown [tokens].
    Known tokens we set:
      [PLATFORM], [APP_VERSION], [timestamp], [app_bundle], [device_make], [device_model],
      [content_language], [IS_LAT], [IFA], [SESSION_ID], [DEVICE_ID]
    Any other [Token] that remains is removed conservatively.
    """
    if not uri:
        return None
    try:
        ...
        # Drop any other bracketed placeholders defensively
        uri = re.sub(r'\[([^]]+)\]', '', uri)
        return uri
    except Exception as e:
...
def get_channels_via_primary_list():
    """Fetch the live channel catalog from Valencia list 10006 and normalize to a compact struct:
       { id, name, number, callsign, logo, group, stream_url=None }
       - Filters out non-live and likely DRM channels.
    """
    ...
    if not data or not isinstance(data, dict):
        logging.warning(f"Failed to fetch valid dictionary data from primary list endpoint.")
        return None

    processed_channels = []
    channel_items = []

    # The list payload sometimes uses data.channel.item[]; sometimes items[] directly.
    if 'channel' in data and isinstance(data['channel'], dict) and 'item' in data['channel'] and isinstance(data['channel']['item'], list):
        ...
        logging.error(f"Could not find channel list in primary list response. Top-level keys: {list(data.keys())}")
        return None

    logging.info(f"Found {len(channel_items)} potential channel items in primary list response.")
    if not channel_items:
        logging.warning("Primary list response contained an empty channel list.")
        return None

    for item in channel_items:
        callsign = item.get('callsign', '')
        properties = item.get('properties', {})
        # properties.is_live appears as string "true"/"false"
        is_live = properties.get('is_live') == "true"
        # simple heuristic to skip DRM streams
        is_drm = callsign.endswith("-DRM") or callsign.endswith("DRM-CMS")
        if is_drm:
            logging.debug(f"Skipping potential DRM channel: {item.get('guid', {}).get('value')} ({item.get('title')})")
            continue
        if not is_live:
            logging.debug(f"Skipping non-live channel: {item.get('guid', {}).get('value')} ({item.get('title')})")
            continue
        ...
        try:
            ...
            # Normalize the logo URL (schema-relative -> https, path-relative -> image host)
            channel_id_str = str(channel_id)
            final_logo_url = None
            if logo_url:
                if logo_url.startswith('//'):
                    final_logo_url = 'https:' + logo_url
                elif logo_url.startswith('/'):
                    final_logo_url = 'https://image.xumo.com' + logo_url
                else:
            ...
            processed_channels.append({
                ...
                'logo': final_logo_url,
                'group': genre,
                'stream_url': None  # will be filled later by stream lookup
            })
        except Exception as e:
            logging.warning(f"Error processing channel list item {item.get('id', 'N/A')}: {e}", exc_info=True)

    if not processed_channels:
        logging.warning("Primary channel list endpoint returned data, but no channels could be successfully processed.")
        return None
...
def fetch_stream_urls_via_asset_lookup(channels_list):
    """For each channel, resolve the *current* asset via the Android TV 'broadcast' endpoint,
    then fetch asset details to locate an HLS (m3u8) source.
    Returns a new list containing only channels where a stream_url was found.
    """
    ...
    for i, channel_info in enumerate(channels_list):
        ...
        # 1) Identify what's on *this hour* so we can grab the asset id
        ...
        if broadcast_data and 'assets' in broadcast_data and isinstance(broadcast_data['assets'], list) and len(broadcast_data['assets']) > 0:
            # Among the returned assets (which may span hours), pick the one actually airing now
            ...
            for asset in broadcast_data['assets']:
                ...
                if start_time and end_time and start_time <= now_utc < end_time:
                    current_asset = asset
                    break
            # Fallback: if we didn’t find a matching window, take the first asset
            ...
            continue  # Skip channel if broadcast fails
        ...
        # 2) Resolve providers/sources for the asset
        ...
        if asset_data and 'providers' in asset_data and isinstance(asset_data['providers'], list):
            # Providers include 'sources': pick an HLS (.m3u8) if possible
                ...
                if ('sources' in provider and isinstance(provider['sources'], list)):
                    for source in provider['sources']:
                        if source.get('uri') and (source.get('type') == 'application/x-mpegURL' or source.get('uri', '').endswith('.m3u8')):
                            ...
                            # Fallback to first available URI if no HLS typed one is found
                            raw_stream_uri = source['uri']
                    if raw_stream_uri:
                        break
        else:
        ...
        # 3) Substitute placeholders and finalize URL
        processed_stream_url = process_stream_uri(raw_stream_uri)
        if processed_stream_url:
            updated_channel_info = channel_info.copy()
            ...
            channels_with_streams.append(updated_channel_info)
            processed_count += 1
        else:
    ...
    return channels_with_streams  # Only channels with stream_url
# <<< END OF FUNCTION ADDED BACK >>>


def fetch_epg_data(channel_list):
    """Fetch consolidated EPG per channel using Valencia hourly EPG by date/hour/offset.
    Strategy:
      - For each date (EPG_FETCH_DAYS) and each hour 0..23:
        - Walk offsets in blocks of 50 until MAX_EPG_OFFSET.
        - Cache page_data['assets'] to avoid per-program asset lookups.
      - Only store schedules for channels present in the final channel list.
      - Deduplicate on (start, assetId) per channel before returning.
    Returns: dict[channel_id] -> list of {start, end, assetId, title, descriptions, episodeTitle}
    """
    if not channel_list:
        return {}
    ...
    assets_cache = {}  # assetId -> asset details
    ...
    for date_obj in dates_to_fetch:
        ...
        # Offsets are paginated; we step by 50 (server-side page size).
        while offset <= MAX_EPG_OFFSET:
            ...
            for hour in range(HOURS_TO_FETCH):
                ...
                if page_data and isinstance(page_data, dict):
                    # Cache the assets dict if present
                    ...
                    if 'channels' in page_data and isinstance(page_data['channels'], list):
                        ...
                        for channel_schedule_data in page_data['channels']:
                            ...
                            if channel_id in channel_ids_in_final_list:
                                found_relevant_channel_in_offset_block = True
                                ...
                                for program_schedule in channel_schedule_data.get('schedule', []):
                                    ...
                                    if asset_details:
                                        ...
                                        # Only store entries with both start and end
    ...
    # Deduplicate per-channel on (start, assetId) to avoid repeats across overlapping pages/hours
    ...
    for channel_id, programs in consolidated_epg.items():
        ...
        for prog in programs:
            ...
            if prog_key not in seen_programs:
                ...
                duplicates_this_channel += 1
        final_epg[channel_id] = unique_programs
        if duplicates_this_channel > 0:
            logging.debug(f"  Channel {channel_id}: Removed {duplicates_this_channel} duplicates. Final count: {len(unique_programs)}")
...
def generate_epg_xml(channel_list_with_streams, consolidated_epg_data):
    """Build an XMLTV tree:
       - One <channel> element per channel in the final M3U set
       - <programme> entries only for channels included in the playlist
    """
    ...
    for channel in channel_list_with_streams:
        ...
        ET.SubElement(chan_el, 'display-name').text = channel['name']  # Only one display-name to avoid duplicates
        if channel['logo']:
    ...
    for channel_id, programs in consolidated_epg_data.items():
        # Skip any program listings not in our playlist (keeps XMLTV clean and aligned to M3U)
        ...
        for program in programs:
            program_asset_id = program.get('assetId', 'N/A')
            ...
            try:
                ...
                stop_formatted = format_xmltv_time(end_time)  # UTC +0000

                if start_formatted and stop_formatted:
                    prog_el = ET.SubElement(
                        tv_element,
                        'programme',
                        attrib={'start': start_formatted, 'stop': stop_formatted, 'channel': channel_id}
                    )
                    ...
                    if asset_id:
                        # Provide an identifier hint for some clients
...
def generate_m3u_playlist(channel_list_with_streams):
    """Create an M3U with #EXTINF lines including tvg-id/name/logo and group-title.
    Channels are sorted numerically by 'number' (if available), then by name.
    """
    ...
    for channel in sorted_channels:
        ...
        display_name = channel['name'].replace(',', ';')  # Commas can break some parsers
        ...
        if stream_url:
            extinf_line = (
                f'#EXTINF:-1 tvg-id="{channel_id}" tvg-name="{tvg_name}" '
                f'tvg-logo="{logo}" group-title="{group_title}",{display_name}\n'
            )
...
if __name__ == "__main__":
    ...
    # Ensure output folder exists before any file writes
    try:
        ensure_output_dir()
    except Exception as e:
    ...
    # Step 1: Channel metadata (names, numbers, logos, genres)
    ...
    # Step 2: For each channel, resolve a playable stream URL via asset lookup (Android TV endpoints)
    ...
    if not final_channel_list_with_streams:
        # If nothing playable resolved, still emit empty, syntactically valid output files
        ...
        # NOTE: save_gzipped_xml no longer exists after switching to plain XML save.
        # This call will raise NameError. Replace with save_xml(...) if you want plain XML output:
        # save_xml(empty_tree, os.path.join(OUTPUT_DIR, EPG_FILENAME))
        # Keeping the original line below with an explanatory comment so you can decide which behavior you want.
        save_xml(empty_tree, os.path.join(OUTPUT_DIR, EPG_FILENAME))  # <-- Likely BUG: function not defined anymore -- fixed by changing function to std .xml
    ...
    # Step 3: Fetch EPG Data (Valencia hourly EPG; caches assets from page payloads)
    ...
    # Step 6: Save files to disk
    save_m3u(m3u_content, os.path.join(OUTPUT_DIR, PLAYLIST_FILENAME))
    save_xml(epg_tree, os.path.join(OUTPUT_DIR, EPG_FILENAME))
